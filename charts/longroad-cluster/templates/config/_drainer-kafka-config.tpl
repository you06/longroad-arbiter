...
detect-interval = {{ .Values.binlog.drainerKafka.detectInterval | default 10 }}
...
txn-batch = {{ .Values.binlog.drainerKafka.txnBatch | default 20 }}
...
worker-count = {{ .Values.binlog.drainerKafka.workerCount | default 16 }}

disable-dispatch = {{ .Values.binlog.drainerKafka.disableDispatch | default false }}

# safe mode will split update to delete and insert
safe-mode = {{ .Values.binlog.drainerKafka.safeMode | default false }}

# downstream storage, equal to --dest-db-type
# valid values are "mysql", "pb", "tidb", "flash", "kafka"
db-type = "{{ .Values.binlog.drainerKafka.destDBType }}"

# disable sync these schema
ignore-schemas = {{ .Values.binlog.drainerKafka.ignoreSchemas | default "INFORMATION_SCHEMA,PERFORMANCE_SCHEMA,mysql" | quote }}
...
{{- if eq .Values.binlog.drainerKafka.destDBType "mysql" }}
# the downstream mysql protocol database
[syncer.to]
host = {{ .Values.binlog.drainerKafka.mysql.host | quote }}
user = {{ .Values.binlog.drainerKafka.mysql.user | default "root" | quote }}
password = {{ .Values.binlog.drainerKafka.mysql.password | quote }}
port = {{ .Values.binlog.drainerKafka.mysql.port | default 3306 }}
...
{{- if eq .Values.binlog.drainerKafka.destDBType "pb" }}
...
{{- if eq .Values.binlog.drainerKafka.destDBType "kafka" }}
...
{{- if .Values.binlog.drainerKafka.kafka.zookeeperAddrs }}
zookeeper-addrs = {{ .Values.binlog.drainerKafka.kafka.zookeeperAddrs | quote }}
{{- end }}
{{- if .Values.binlog.drainerKafka.kafka.kafkaAddrs }}
kafka-addrs = {{ .Values.binlog.drainerKafka.kafka.kafkaAddrs | quote }}
{{- end }}
kafka-version = {{ .Values.binlog.drainerKafka.kafka.kafkaVersion | default "0.8.2.0" | quote }}
